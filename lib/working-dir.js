...
export async function getDatManifest (dir) {
  ...
  try {
    ...
    throw new Error(`Invalid dat.json file, aborting. (${e.toString()})`)
  }
  return manifest
}

export async function getDatIgnore (dir) {
  try {
    return (await fs.readFile(path.join(dir, '.datignore'), 'utf8'))
      .split('\n')
      .filter(str => !!str.trim())
      .map(line => path.join(dir, line)) // prepend the dir to each line
  } catch (e) {
    return ''
  }
}
